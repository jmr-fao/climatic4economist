...
cat("Number of mismatch in adm div 2:", 
...
# tmp_cntry <- purrr::map2(adm_div,
#                          names(adm_div),
#                          crop_with_buffer,
#                          raster = tmp,
#                          buffer = 1,
#                          filename = "tmp_crop.nc",
#                          overwrite = TRUE)
# tmp_tza <- crop_with_buffer(raster = tmp,
#                             vector = adm_div$TZA,
#                             buffer = 1,
#                             filename = "tanzania_crop.nc",
#                             overwrite = TRUE)
...
tmp_coord$Uganda16 <- extract_by_poly(tmp, adm_div$UGA, fn_agg = "mean") |> 
  dplyr::rename(ID = ID_adm_div)
pre_coord$Uganda16 <- extract_by_poly(pre, adm_div$UGA, fn_agg = "mean") |> 
  dplyr::rename(ID = ID_adm_div)
...
# From meter to millimeters per day
pre_coord_mm <- purrr::map(
  pre_coord,
  \(x) dplyr::mutate(x, 
                     dplyr::across(
...
saveRDS(tmp_coord_c, file = file.path(path_to_temp, "tmp_coord.rds" ))
saveRDS(pre_coord_mm, file = file.path(path_to_temp, "pre_coord.rds" ))
```
