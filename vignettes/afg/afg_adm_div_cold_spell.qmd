---
title: "Cold Spell in Afganistan Districts"
author: "JMR"
toc: true
toc-expand: 1
toc-depth: 2
...
>These percentiles were already computed in another tutorial so we directly load them. Similarly, we already extracted the weather observation based on the administrative divisions in the same tutorial, so we directly load them as well.

When working with administrative divisions there are two approaches. The first approach spatially average the weather observations within each area and then calculate the percentile. The second approach calculates the percentiles for each spatial points from the weather data. The first approach is faster as it requires less memory, but it is less accurate because it average the observations. Indeed, by averaging we smooth the distribution and we lessen the role of extreme weather realizations.

We use ERA5-Land temperature observations and the administrative divisions of Afghanistan.
...
1.  Load the data
2.  Find the weather extreme events with absolute threshold
3.  Find the weather extreme events with relative threshold
4.  Compute the weather extreme indicators
5.  Merge the weather extreme indicators with the survey
6.  Save the result
...
```{r set_up}
#| label: set_up
path_to_tmp_min <- file.path("data", "weather", "ERA5_Land", "AFG", 
                             "daily", "day_50_25_2m_tmp_min.rds")
path_to_tmp_min_percentile <- file.path("data", "weather", "ERA5_Land", "AFG", 
                                        "percentile",  "pct_80_25_2m_tmp_min.rds")

path_to_cold_indices <- file.path("data", "result", "afg_adm_cold_ind_2024_09.dta")
```
...
### Load the data
Let's start by loading the data into R. We load the weather observation with the function `readRDS()` as file as saved as `rds` format.

We notice that both datasets have some variables in common: `ID_adm_div`, `x`, `y`, `coverage_fraction`, `ADM1NM`, and `ADM2NM`.
...
tmp_min_div <- readRDS(path_to_tmp_min)|>
  dplyr::as_tibble()
tmp_min_div
...
tmp_min_pct |> 
  dplyr::filter(ADM2CD == "AFG001004") |>
  dplyr::group_by(month) |>
  dplyr::summarise(dplyr::across(.cols = c(day_1p, day_5p, day_10p),
                                 .fns = mean))
```

As we mention in the [Intro] there are two approaches when working with administrative divisions. One is faster but less accurate and consist of spatially average all the weather observations within the divisions as a first step, i.e. at the extraction phase. The other keeps all the weather observations separately, performs the weather extreme indicators for each of them, and only at the last phase it spatially averages them. 

As you could see form the load data, we are going for this second approach. Indeed, we have both the `ID_adm_div` and the `ID` columns. Since we will compute the indicators for each weather observation we will use the `ID` column as reference. However, this column is absent for the weather observation. We can add it using the function `prepare_coord()` and by saying which are the unique coordinates of the weather observations.
...
tmp_min_div_23 <- select_by_dates(tmp_min_div, 
...
cold_day_abs <- find_extr_abs_day(tmp_min_div_23,
                                  l_thresh = c(0, -15, -20),
                                  unit = "celsius")
cold_day_abs
```
...
cold_day_rel <- find_extr_rel_day(tmp_min_div_23,
...
### Merge with administrative division and aggregate
Now that we have all the extreme weather indicators, we need to aggregate them for each administrative division. To do so, we need first to merge the extreme weather indicator with the administrative divisions. We achieve this using the function `merge_with_survey()`, which matches records based on the `ID` column.

```{r merge_ext_event}
#| label: merge_abs_ext_event

cold_day_abs_div <- merge_with_survey(tmp_min_div, cold_abs_index) 
cold_day_abs_div

cold_spell_abs_div <- merge_with_survey(tmp_min_div, cold_spell_abs_index)
cold_spell_abs_div

cold_day_rel_div <- merge_with_survey(tmp_min_div, cold_rel_index)
cold_day_rel_div

cold_spell_rel_div <- merge_with_survey(tmp_min_div, cold_spell_rel_index)
cold_spell_rel_div
```

Now we aggregate by administrative division. Note how the number of rows is reduced. The function `agg_to_adm_div()` recognize the administrative divisions only by `ID_adm_div`, so by default the function will discard any other information on the administrative divisions. If we want to retain other columns we can do it with the argument `extra_col`. 

```{r agg_adm_div}
#| label: agg_adm_div

cold_day_abs_agg <- agg_to_adm_div(cold_day_abs_div,
                                   extra_col = c(ADM1CD, ADM1NM,
                                                 ADM2CD, ADM2NM))
cold_day_abs_agg

cold_spell_abs_agg <- agg_to_adm_div(cold_spell_abs_div,
                                     extra_col = c(ADM1CD, ADM1NM,
                                                   ADM2CD, ADM2NM))
cold_spell_abs_agg

cold_day_rel_agg <- agg_to_adm_div(cold_day_rel_div,
                                   extra_col = c(ADM1CD, ADM1NM,
                                                 ADM2CD, ADM2NM))
cold_day_rel_agg

cold_spell_rel_agg <- agg_to_adm_div(cold_spell_rel_div,
                                     extra_col = c(ADM1CD, ADM1NM,
                                                   ADM2CD, ADM2NM))
cold_spell_rel_agg
```
...
### Write
Now that we have the cold spell indices we can save the results as `dta` using the `haven::write_dta()` function. To avoid having too many datasets, we first merge all the different indicators. Then we adapt the name to be aligned with the STATA format, we delate the fifth column as the name cannot be aligned with the STATA format, and finally we save the result.

```{r write}
#| label: write

list(cold_day_abs_agg, cold_day_rel_agg, cold_spell_abs_agg, cold_spell_rel_agg) |>
  purrr::reduce(dplyr::full_join,
                by = c("ID_adm_div", "ADM1CD", "ADM1NM", "ADM2CD", "ADM2NM", "lag")) |>
