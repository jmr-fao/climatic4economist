...
srvy_all <- list.files(path_to_survey, pattern = "cooridnates\\.dta$", full.names = TRUE) |>
  lapply(haven::read_dta) |>
  lapply(\(x) dplyr::select(x, 
                            dplyr::any_of(c("country",  "year",
                                            "hhid", "lat", "lon", "interview_date")))) |>
  dplyr::bind_rows() |> 
...
pop <- list.files(path_to_pop, full.names = TRUE) |>
  lapply(terra::rast) |>
  setNames(c("pop_2016", "pop_2017", "pop_2018", "pop_2019", "pop_2020")) |>
  terra::rast()

nightlight <- list.files(path_to_nightlight, pattern = "20[12][67891].*tif$", full.names = TRUE) |>
  lapply(terra::rast) |>
  setNames(c("nightlight_2016", "nightlight_2017", "nightlight_2018", "nightlight_2019", "nightlight_2021")) |>
  terra::rast()
